...
export const getWallet = async () => {
  ...
  const wallets = await provider.send('ton_requestWallets')
  return wallets[0]
}
...
export class Peer {
  ...
  async signNewChannelState (newChannelState) {
    // A signs this state and send signed state to B (e.g. via websocket)

    const signature = await this.channel.signState(newChannelState)
    return signature
